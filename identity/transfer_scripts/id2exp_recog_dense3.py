...
class Transfer(nn.Module):
    def __init__(self, nClasses):
        ...
        self.fc = nn.Linear(1*1*184, nClasses) #conv 1 147456 # lay1 46080, lay2 12672, lay 3 26496.  368640 101376. 
    def forward(self,x):
        #x = x.view(-1,2048)
        #print(x.shape)
        #x = x.view(x.size(0),-1)
...
class KDEFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = self.dataInfo.iloc[idx, 8] #6 conv1 
        label = self.dataInfo.iloc[idx, 1] #exp
...
ids = ['idset1', 'idset2', 'idset3', 'idset4', 'idset5', 'idset6', 'idset7', 'idset8', 'idset9', 'idset10']
accs_all = list()
accs_id_mean = list()
count = 0
for i, id_set in enumerate(ids):
    for rep in range(1,11):
        accs_ids = list()
        path = '/mmfs1/data/schwarex/neuralNetworks/densenet/retrains/identity/kdef/kdef_features_merged_%s_%d.pkl' % (id_set, rep)
        ...
        accs_ids.append(meanAccuracy)
    accs_id_mean.append(mean(accs_ids))
...
for val in range(0,10):
    ...
    avg_1.append(accs_all[val+30])
    avg_1.append(accs_all[val+40])
    avg_1.append(accs_all[val+50])
    avg_1.append(accs_all[val+60])
    avg_1.append(accs_all[val+70])
    avg_1.append(accs_all[val+80])
    avg_1.append(accs_all[val+90])
