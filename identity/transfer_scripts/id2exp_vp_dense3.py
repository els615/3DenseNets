...
class KDEFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = self.dataInfo.iloc[idx, 9] #6 conv1 
        label = self.dataInfo.iloc[idx, 1] #id
...
for i, view in enumerate(angles):
    for rep in range(1,11):
        accs_angle = list()
        path = '/mmfs1/data/schwarex/neuralNetworks/densenet/retrains/identity_v2/kdef/kdef_features_merged_%s_%d.pkl' % (view, rep)
